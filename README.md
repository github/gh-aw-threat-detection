...
That makes *any* recorded `ERR_VALIDATION` finding — including one about an
optional channel such as an uninspectable `comment-memory` directory — an error
...
| `GH_AW_DETECTION_CONTINUE_ON_WARNING` | Optional, host-integrated runs | Anything other than `"false"` (compared case-insensitively) continues, which is the default. `"false"` makes *any* uninspectable artifact channel — including optional ones like `comment-memory` — a `config_error` (exit `2`) before the engine runs. Separate from `GH_AW_DETECTION_CONTINUE_ON_ERROR`; not consumed by `conclude`. |
