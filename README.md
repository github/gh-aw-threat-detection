...
### Testing the Latest Detector Under AWF (`refresh-latest-smokes.yml`)
...
These locks are regenerated by `.github/workflows/refresh-latest-smokes.yml`. On each run it:

1. Resolves the newest **version release** of both `github/gh-aw` and `github/gh-aw-threat-detection` — the most recent by publish date whether it is a prerelease or a stable release, so a newer stable wins over an older prerelease and vice versa. Rolling refs such as the detector's `main` tag are ignored so the pinned locks stay reproducible. Both are overridable via inputs.
2. Clones gh-aw at that ref, patches `DefaultThreatDetectVersion` to the resolved detector tag, and builds the `gh-aw` compiler from source.
3. Recompiles the three `smoke-<engine>-standalone-latest.md` sources with that patched compiler using SHA-pinned action mode (`--action-mode action --action-tag <gh-aw release tag>`), so the **only** thing that differs from production is the detector version.
4. Opens (or updates) a pull request with the regenerated `.lock.yml` files.

This is a deliberately simple, fully reviewable two-step flow — **open the PR, eyeball the version diff, merge, then the smokes run** — with no throwaway branches or cross-branch dispatch. `refresh-latest-smokes.yml` runs:

- **Automatically** after the **Release** workflow finishes publishing a new (pre)release (`workflow_run`). The Release workflow creates the release with `GITHUB_TOKEN`, so a plain `on: release` trigger would never fire — hence the `workflow_run` hook. On auto-runs the detector ref defaults to the just-published tag when it is a version tag (otherwise the newest detector version release).
- **Manually** via `workflow_dispatch`, with optional `gh_aw_ref` and `detector_ref` inputs.

The `smoke-<engine>-standalone-latest` workflows themselves are **dispatch-only** plus a `push` trigger scoped to their own `.lock.yml` on `main`: merging a refresh PR updates a lock, which triggers that engine's latest smoke automatically. They never run on a schedule, so the pinned `*-standalone` smokes' steady-state behaviour is unchanged.

**Tag → test → promote loop:**

1. Cut a release tag (`create-release-tag.yml`); `release.yml` publishes a version-tagged **prerelease** with the recorded asset sha256. `refresh-latest-smokes.yml` then runs automatically and opens a PR pinning the latest locks to that tag.
2. To re-run or pin explicit refs manually, dispatch **Refresh Latest Smokes** (`workflow_dispatch`) with a `detector_ref` and/or `gh_aw_ref` (or leave them empty).
...
>
> The `smoke-<engine>-standalone-latest.lock.yml` files are **generated** by this workflow, not by a plain `gh aw compile` — running the normal compiler over their `.md` would revert the patched detector version. Regenerate them by re-running **Refresh Latest Smokes**.
