...
command. The diagnostics MUST be written to stderr and recorded in the run log
(TD-20a) as an `engine_preflight` event when enabled; per TD-20c they MUST NOT
...
only, the engine preflight (TD-20j) through stderr and the run log, and the
conclusion verdict through the run log and the `conclude`
