...
The detector version **is** pinned in the locks: gh-aw's `buildInstallThreatDetectStep` emits `constants.DefaultThreatDetectVersion` (a hardcoded tag) as the argument to `install_threat_detect_binary.sh`. There is no compile-time override. The pin only moves when the upstream constant is bumped (typically after a detector promotion) and this repo's locks are recompiled against that gh-aw tag — promoting a detector release is not by itself enough. Unpromoted prereleases are never picked up automatically; test those via `replay-detection.yml` with `detector_source=release` and `detector_ref=<prerelease tag>`, or, when you specifically need the fresh-AWF smoke path, via the targeted post-compile `sed` documented in [`skills/update-workflow-versions/SKILL.md`](skills/update-workflow-versions/SKILL.md).
